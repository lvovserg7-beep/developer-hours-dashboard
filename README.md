...
Стабильная версия: [v2.2.0](https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.2.0)
...
Вкладка **Доходы и расходы** собирается отдельно, по кнопке «Сформировать», без кэша часов. Формулы как в отчёте 1С: выручка и себестоимость — регистр «Выручка и себестоимость продаж» (ОП/КО по дате «ОкончанияБонусовОП» у контрагента), статьи — справочник «НастройкаДИР», расходы — регистр «Прочие расходы», прочие доходы — регистр «Прочие доходы». Закрытые часы в «Справочно» — количество по номенклатуре настройки с порядком 1, не задачи разработчика. Организация — «Аллсан Интеграция». По умолчанию период как в 1С: с начала месяца три месяца назад по конец текущего.
...
https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.2.0

В PowerShell:

```powershell
cd C:\Apps
git clone --branch v2.2.0 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
```
...
Рабочая папка в Cursor и боевой каталог — разные копии. На боевую **не** копировать архив с рабочего стола. Берут стабильный тег из этого руководства (файл `STABLE`, сейчас **v2.2.0**).
...
Скрипт берёт тег из файла `STABLE`, подтягивает его с GitHub и перезапускает сервер. `.env` и `users.json` git не трогает. Другой тег вручную: `update-stable.cmd v2.2.0`.
...
sudo mkdir -p /opt
cd /opt
sudo git clone --branch v2.2.0 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
...
Тег из `STABLE`, либо явно: `./update-stable.sh v2.2.0`. Если сервис systemd активен — скрипт перезапустит его; иначе запустите снова `./start.sh`.
