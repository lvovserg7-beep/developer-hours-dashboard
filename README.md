...
Стабильная версия: [v2.8.0](https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.8.0)
...
В обычном режиме данные не обновляются сами: отчёты — по кнопке «Сформировать», часы и активность — при открытии вкладки. Вход по логину и паролю. Вкладки: **Часы**, **Активность**, **Доходы и расходы** (Аллсан и Первый интегратор), **Сводка юнитов**, **Исполнение плана**, **Бюджет**, **Bitrix**, **Чистота ведения Битрикс**, **Озон ДРР**, **Отгрузки ФБС ozon**, **Отгрузки ФБО Озон**, **Поставки ФБО с фильтрами**, **WB**, **Поставки ФБС WB**, **Задолженность клиентов**, **Управленческий баланс**, **Реестр оплат клиентов**, **Качество работы с клиентами**, **Качество работы с клиентами TV**, SEO-доски (на «Поиск SEO» — ИКС Яндекса), у администратора — **Админка** и **Кэш** (очистка и перечитывание данных за выбранный период). При старте перечитывается SEO-кэш за последние 7 дней; первый вход за день — SEO за вчера. `start.cmd` при выходе процесса перезапускает сервер. Режим «На весь экран» с перелистыванием слайдов (кнопки и ← →); в нём данные слайдов обновляются по таймеру. Клик по номеру задачи копирует навигационную ссылку 1С. На самой странице адрес сервиса 1С не показывается. Перед запуском `start.cmd` / `start.sh` сам останавливает старый процесс на порту 8787. На Linux та же программа: `dashboard/start.sh` или systemd (см. ниже).
...
- Задачи с флагом **Попадос** — не клиентские; в выполненные часы (в т.ч. по клиентам) не входят.

Вкладка **Доходы и расходы** собирается отдельно, по кнопке «Сформировать», без кэша часов. Формулы как в отчёте 1С: выручка и себестоимость — регистр «Выручка и себестоимость продаж» (весь месяц продажи — ОП, если дата «ОкончанияБонусовОП» у контрагента не раньше 1-го числа этого месяца), статьи — справочник «НастройкаДИР», расходы — регистр «Прочие расходы», прочие доходы — регистр «Прочие доходы». Закрытые часы в «Справочно» — количество по номенклатуре настройки с порядком 1, не задачи разработчика. Организация — «Аллсан Интеграция». По умолчанию период как в 1С: с начала месяца три месяца назад по конец текущего.

Вкладка **ДИР Первый интегратор** — типовой отчёт 1С из базы ecotidy (ПЕРВЫЙ ИНТЕГРАТОР ООО). Комиссионное вознаграждение берётся из ресурса «Расходы на продажу»; в прочие расходы входят только статьи с вариантом распределения «на направления деятельности» или «не распределять» (статьи на себестоимость товаров, в том числе «Сертификация», в отчёт не попадают).
...
https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.8.0

В PowerShell:

```powershell
cd C:\Apps
git clone --branch v2.8.0 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
```
...
Пользователи хранятся в `dashboard\users.json` (в git не попадает). В админке можно завести обычных пользователей, включить им вкладки **Часы**, **Активность**, **Доходы и расходы** и выдать права администратора.
...
Рабочая папка в Cursor и боевой каталог — разные копии. На боевую **не** копировать архив с рабочего стола. Берут стабильный тег из этого руководства (файл `STABLE`, сейчас **v2.8.0**).
...
Скрипт берёт тег из файла `STABLE`, подтягивает его с GitHub и перезапускает сервер. `.env` и `users.json` git не трогает. Другой тег вручную: `update-stable.cmd v2.8.0`.
...
sudo mkdir -p /opt
cd /opt
sudo git clone --branch v2.8.0 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
...
Подробно: [`dashboard/ОБНОВЛЕНИЕ.md`](dashboard/ОБНОВЛЕНИЕ.md).

```bash
/opt/developer-hours-dashboard/dashboard/update-stable.sh
```

Тег из `STABLE`, либо явно: `./update-stable.sh v2.8.0`. Если сервис systemd активен — скрипт перезапустит его; иначе запустите снова `./start.sh`.
