...
Стабильная версия: [v1.12.2](https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v1.12.2)
...
https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v1.12.2

В PowerShell:

```powershell
cd C:\Apps
git clone --branch v1.12.2 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
```
...
Рабочая папка в Cursor и боевой каталог — разные копии. На боевую **не** копировать архив с рабочего стола. Берут стабильный тег из этого руководства (файл `STABLE`, сейчас **v1.12.2**).
...
Скрипт берёт тег из файла `STABLE`, подтягивает его с GitHub и перезапускает сервер. `.env` и `users.json` git не трогает. Другой тег вручную: `update-stable.cmd v1.12.2`.
