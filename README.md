...
Стабильная версия: [v2.3.0](https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.3.0)
...
https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.3.0

В PowerShell:

```powershell
cd C:\Apps
git clone --branch v2.3.0 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
```
...
Рабочая папка в Cursor и боевой каталог — разные копии. На боевую **не** копировать архив с рабочего стола. Берут стабильный тег из этого руководства (файл `STABLE`, сейчас **v2.3.0**).
...
Скрипт берёт тег из файла `STABLE`, подтягивает его с GitHub и перезапускает сервер. `.env` и `users.json` git не трогает. Другой тег вручную: `update-stable.cmd v2.3.0`.
...
sudo mkdir -p /opt
cd /opt
sudo git clone --branch v2.3.0 https://github.com/lvovserg7-beep/developer-hours-dashboard.git
...
Тег из `STABLE`, либо явно: `./update-stable.sh v2.3.0`. Если сервис systemd активен — скрипт перезапустит его; иначе запустите снова `./start.sh`.
