...
Стабильная версия: [v2.1.1](https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.1.1)
...
https://github.com/lvovserg7-beep/developer-hours-dashboard/releases/tag/v2.1.1
...
Рабочая папка в Cursor и боевой каталог — разные копии. На боевую **не** копировать архив с рабочего стола. Берут стабильный тег из этого руководства (файл `STABLE`, сейчас **v2.1.1**).
...
Скрипт берёт тег из файла `STABLE`, подтягивает его с GitHub и перезапускает сервер. `.env` и `users.json` git не трогает. Другой тег вручную: `update-stable.cmd v2.1.1`.
...
Тег из `STABLE`, либо явно: `./update-stable.sh v2.1.1`. Если сервис systemd активен — скрипт перезапустит его; иначе запустите снова `./start.sh`.
