...
export async function loadPnlEcotidy(fromText, toText, groupBy = "month") {
  ...
  return {
    ...
    totals: {
      ...
      commission: roundMoney(commission.reduce((s, v) => s + v, 0)),
      ...
      byMonth: months.map((m, i) => ({
        ...
        commission: roundMoney(commission[i]),
        sales: roundMoney(salesProfit[i]),
        otherIncome: roundMoney(incomeVec[i]),
        otherExpenses: roundMoney(expenseVec[i]),
        net: roundMoney(net[i]),
      })),
    },
